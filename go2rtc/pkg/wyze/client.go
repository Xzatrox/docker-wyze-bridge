...
type Client struct {
	...
	authKey string
	verbose bool
	// notify is true when the wyze:// URL carried &notify=true.
	// When set, the producer will install a NotifyFunc on the DTLSConn
	// to capture unsolicited camera IOCTRL notifications (e.g. doorbell
	// ring) and emit them as WYZE-NOTIFY lines on stdout.
	notify bool
...
// Notify returns true when the caller requested live IOCTRL notifications
// via the &notify=true URL parameter. The Producer uses this to decide
// whether to install the NotifyFunc on the DTLSConn.
func (c *Client) Notify() bool {
	return c.notify
}
