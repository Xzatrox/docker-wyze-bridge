...
type DTLSConn struct {
	...
	// passed so the caller can classify the notification without
	// needing to re-parse the HL header.
	// Called from the worker goroutine; must not block.
	NotifyFunc func(cmdID uint16, hlPayload []byte)
}
...
// dispatchNotify parses an unsolicited IOCTRL / channel-message frame
// and, if NotifyFunc is set, calls it with the parsed HL command ID and
// payload. Safe to call from the worker goroutine; NotifyFunc must not
// block or call back into DTLSConn methods.
func (c *DTLSConn) dispatchNotify(data []byte) {
	fn := c.NotifyFunc
	if fn == nil {
		return
	}
	// Try the two known HL offsets (same logic as client.extractHL).
	var hlData []byte
	if h := tutk.FindHL(data, 32); h != nil {
		hlData = h
	} else if len(data) >= 36 && data[16] == 0x00 {
		hlData = tutk.FindHL(data, 36)
	}
	if hlData == nil {
		return
	}
	cmdID, payload, ok := tutk.ParseHL(hlData)
	if !ok {
		return
	}
	fn(cmdID, payload)
